...
create table vector_advertising_models (
  ...
  created_by text not null, created_at timestamptz not null default now(), unique (organization_id, name, version)
);
create table vector_christian_reports (
  id text primary key, organization_id text not null references organizations(id), source_message_id text not null,
...
create table vector_product_identities (
  id text primary key, organization_id text not null references organizations(id), marketplace text not null,
...
create table vector_advertising_evidence (
  id text primary key, organization_id text not null references organizations(id), product_identity_id text not null,
  decision_case_id text references decision_cases(id), source_evidence_id text, classification text not null,
...
create table vector_forecasts (
  id text primary key, organization_id text not null references organizations(id), product_identity_id text not null,
  decision_case_id text references decision_cases(id), model_id text not null references vector_advertising_models(id),
...
create table vector_interventions (
  id text primary key, organization_id text not null references organizations(id), product_identity_id text not null,
  decision_case_id text references decision_cases(id), action text not null, current_state jsonb not null,
...
create table joint_decision_cases (
  id text primary key, organization_id text not null references organizations(id), decision_case_id text references decision_cases(id),
  product_identity_id text not null, oracle_planning_case_id text, status text not null default 'DRAFT',
...
create table joint_safe_demand_envelopes (
  id text primary key, organization_id text not null references organizations(id), joint_case_id text not null,
...
create table joint_dependency_events (
  id text primary key, organization_id text not null references organizations(id), source_domain text not null,
...
create table joint_causal_edges (
  id text primary key, organization_id text not null references organizations(id), joint_case_id text,
