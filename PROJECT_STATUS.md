...
The `agent/apple-experience-redesign` branch extends that visual system across the authenticated shell, navigation glyphs, search, popovers, Atlas, Mercury states, and sign-in. It reuses the active token family and existing components; routes, permissions, backend requests, authentication, persistence, and the marketing site remain unchanged. This broader frontend pass is **implemented locally** and must not be labeled production-verified until its validation and deployment are complete.

The `agent/apple-system-themes` follow-up implements the complete guide-aligned theme contract: light and dark palettes selected by the operating-system preference, adaptive brand assets, Apple blue for functional actions and focus, centralized surface/shadow/radius tokens, and mode-safe Decision Lab, Mercury, Atlas, shell, popover, and sign-in components. It is frontend-only and remains **implemented locally / production unverified** until validation and deployment complete.

The `agent/apple-composition-pass` follow-up corrects the application hierarchy to match the supplied guide rather than relying on theme tokens alone: compact 34–48px page headlines, solid mode-aware cards, a quieter 232px sidebar, 8-point spacing, reduced surface density, restrained radii and shadows, and responsive two- and one-column collapse. No data, route, permission, or backend contract changes are included.
...
The `agent/vector-joint-decision-engine` release adds the Media Diagnostics and Oracle–Vector coordination foundation: additive migration `011`, evidence-only Christian report ingestion, an independent Vector model, Safe Demand Envelopes, explicit cross-domain invalidation, possible futures, disagreement preservation, per-model outcomes, centralized RBAC, Mercury joint context, `/vector`, and `/api/vector/*`. Local validation passed. No Amazon Ads reader, Outlook connector, margin source, or provider mutation adapter is connected; therefore no production advertising action or economic result is claimed.
